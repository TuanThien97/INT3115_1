...
namespace common\templates\model;
...
class Generator extends \yii\gii\Generator
{
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            // base
            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/db/' . $modelClassName . '.php',
            ...
            // api
            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->apiNs)) . '/' . $modelClassName . '.php',
                $this->render('api.php', $params)
            );

            // console
            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->consoleNs)) . '/' . $modelClassName . '.php',
                $this->render('console.php', $params)
            );

            // backend
            // $files[] = new CodeFile(
            //     Yii::getAlias('@' . str_replace('\\', '/', $this->backendNs)) . '/' . $modelClassName . '.php',
            //     $this->render('backend.php', $params)
            // );

            // // frontend
            // $files[] = new CodeFile(
            //     Yii::getAlias('@' . str_replace('\\', '/', $this->frontendNs)) . '/' . $modelClassName . '.php',
            //     $this->render('frontend.php', $params)
            // );
